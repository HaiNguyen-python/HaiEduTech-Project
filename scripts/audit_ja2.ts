...
for (const g of [...VOCAB_TOPICS, ...VOCAB_EXTRA, ...VOCAB_EXTRA_2, ...VOCAB_EXTRA_3] as any[])
...
for (const k of [...KANJI_GROUPS.flatMap((g:any)=>g.items), ...KANJI_EXTRA, ...KANJI_EXTRA_2, ...KANJI_EXTRA_3] as any[]) {
...
const allQ = [...JA_QUIZ, ...JA_QUIZ_EXTRA, ...JA_QUIZ_EXTRA_2, ...JA_QUIZ_EXTRA_3] as any[];
...
const dlgs = [...DIALOGUES_EXTRA, ...DIALOGUES_EXTRA_2, ...DIALOGUES_EXTRA_3] as any[];
for (const d of dlgs) d.lines.forEach((l:any)=>{ if(!l.romaji||!l.vi||!l.en) bad(`dlg missing ${d.title} ${l.jp}`); });
for (const g of [...GRAMMAR_EXTRA, ...GRAMMAR_EXTRA_2, ...GRAMMAR_EXTRA_3] as any[]) if(!g.examples?.length) bad(`grammar no ex ${g.title}`);
