...
Giai đoạn 3: `ReactionPicker.tsx` (đóng trễ 400ms, hover bridge, nút 40x40, long-press trên mobile, Escape/click ngoài, mũi tên), `ReactorsDialog.tsx` xem ai đã thả cảm xúc (RPC `get_post_reactors`), `your_corner_posts.is_question` (nhãn "Đang chờ trả lời" khi chưa có bình luận), `your_corner_comments.is_helpful` (chủ bài hoặc staff đánh dấu), nút nghe phát âm bình luận bằng speechSynthesis, `HelpersCard.tsx` bảng người giúp đỡ 7 ngày (RPC `get_your_corner_helpers`, hữu ích x5 + tim). Đã sửa lỗi nhân đôi `reaction_count` trong `get_your_corner_feed`.
