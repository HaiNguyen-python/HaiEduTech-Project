...
- No double reading: one fresh `Audio` element per turn, handlers detached when leaving a turn, `AbortError`/`NotAllowedError` from `play()` never triggers the device-voice fallback, one settled outcome per turn, and `speak()` ignores calls while the recording is still downloading.
