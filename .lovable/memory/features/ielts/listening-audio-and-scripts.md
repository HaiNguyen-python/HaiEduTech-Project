...
- Audio: edge function `listening-tts` (verify_jwt false) generates `openai/gpt-4o-mini-tts` mp3 per speaker turn and caches them in the private `listening-audio` bucket, returning 6h signed URLs. Voice per speaker comes from `src/lib/ieltsListeningVoices.ts`. AI voice is always on (no toggle, no `ielts-listening-ai-voice` flag); browser speechSynthesis is a silent fallback shown as "Using device voice fallback".
- No double reading: one fresh `Audio` element per turn, handlers detached when leaving a turn, `AbortError`/`NotAllowedError` from `play()` never triggers the device-voice fallback, one settled outcome per turn, and `speak()` ignores calls while the recording is downloading or already playing. A turn claims its outcome before awaiting a signed-URL refresh, and only one section player may be active across a full test.
