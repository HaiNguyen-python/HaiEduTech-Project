...
- Practice sentence text must be unique per language: `src/data/speakingCoachTextFixes.ts` rewrites duplicated sentences keyed by their FINAL post-dedupe id (only `text`/`translation`/`ipa` change, ids stay so saved scores survive), applied in `buildThemes` after `dedupeSpeakingThemes` + `mergeSpeakingThemes`. `scripts/audit_speaking_coach.ts` treats duplicate sentence text as a blocking error and `src/test/speakingCoach.test.ts` guards it.
- `ConversationScrollButton` (ai-elements/conversation.tsx) and `WeakWordReview` are `forwardRef` components to avoid React ref warnings.
