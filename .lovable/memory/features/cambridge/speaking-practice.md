...
description: Cambridge Starters-PET speaking module - 101 tasks, AI exam pictures, recorder hardening, star grading
type: feature
---
Route `/cambridge-speaking-practice` (menu Cambridge Starters -> PET + button on Cambridge Lectures).

- Task bank: `src/data/cambridgeSpeakingTasks.ts` (core) + `cambridgeSpeakingTasksExpansion.ts` + `cambridgeSpeakingTasksExpansion2.ts` (50 extra, 10 per level) + `cambridgeSpeakingTasksExpansion3.ts` (50 extra, 10 per level; 151 tasks total). 20+ tasks per level across Starters/Movers/Flyers/KET/PET, real exam part formats with examiner follow-ups, useful language and model answers.
...
- UI rules: no Vietnamese subtitle under the exam prompt (English only). The prompt is split by sentence so each box holds exactly one question, numbered, with a per-question listen button. Examiner follow-ups render one per box. Duplicate topic chip labels get a numeric suffix.
