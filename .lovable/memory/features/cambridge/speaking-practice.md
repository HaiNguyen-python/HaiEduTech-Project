...
- Task bank: `src/data/cambridgeSpeakingTasks.ts` (core) + `cambridgeSpeakingTasksExpansion.ts` + `cambridgeSpeakingTasksExpansion2.ts` (50 extra, 10 per level). 20+ tasks per level across Starters/Movers/Flyers/KET/PET, real exam part formats with examiner follow-ups, useful language and model answers.
