// English TTS proxy - relays Google Translate audio for en-US/en-GB so the
// browser doesn't have to fight CORS or rely on system voices being installed.
// Mirrors swedish-tts / finnish-tts / vietnamese-tts.
const corsHeaders = {
...
const endpointBuilders = (tl: string) => [
  (text: string) =>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const accent = body?.accent === "en-GB" ? "en-gb" : "en";
    ...
    for (const buildEndpoint of endpointBuilders(accent)) {
    ...
    return new Response(JSON.stringify({ error: "Unable to fetch English audio" }), {
      status: 502,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
