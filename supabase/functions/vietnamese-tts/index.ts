// Vietnamese TTS proxy — uses Google Translate TTS (giọng Việt tự nhiên, miễn phí, không cần API key)
const corsHeaders = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Unable to fetch Vietnamese audio" }), {
      status: 502,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("vietnamese-tts error", error);
