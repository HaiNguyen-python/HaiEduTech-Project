// Chinese TTS proxy - relays Google Translate audio for zh-CN. Critical for
// HSK / Mandarin lessons because many browsers ship without zh-CN voices.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Unable to fetch Chinese audio" }), {
      status: 502,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
