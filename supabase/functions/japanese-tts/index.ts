// Japanese TTS proxy — relays Google Translate audio for ja-JP so the browser
// doesn't have to fight CORS. Mirrors swedish-tts / chinese-tts.
const corsHeaders = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Unable to fetch Japanese audio" }), {
      status: 502,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("japanese-tts error", error);
