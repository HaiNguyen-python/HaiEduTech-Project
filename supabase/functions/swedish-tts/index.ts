// Swedish TTS proxy — relays Google Translate audio for sv-SE so the browser
// doesn't have to fight CORS. Mirrors the finnish-tts edge function.
const corsHeaders = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Unable to fetch Swedish audio" }), {
      status: 502,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("swedish-tts error", error);
