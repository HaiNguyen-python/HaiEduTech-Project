  ...
  if (finished) {
    return (
      <div className="text-center p-6 rounded-2xl bg-gradient-to-br from-teal-500/10 to-violet-500/10 border-2 border-teal-400/40">
        ...
        <h4 className="font-display font-bold text-lg">Xong tình huống! 🌟</h4>
        <p className="text-sm text-muted-foreground mb-3">
          Lựa chọn đúng: <b>{score}/{questions.length}</b>
        </p>
        <Button size="sm" variant="outline" onClick={reset}>
          <RotateCcw className="w-3 h-3 mr-1" /> Chơi lại
        </Button>
      </div>
    );
  }
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-bold uppercase tracking-wider text-teal-600 dark:text-teal-400">
          🎬 Tình huống {qIdx + 1}/{questions.length}
        </span>
      </div>
      ...
      {pickedChoice && pickedMeta && (
        ...
        >
          ...
          >
            {qIdx + 1 < questions.length ? "Tình huống tiếp →" : "Xem kết quả"}
          </Button>
        </motion.div>
      )}
    </motion.div>
