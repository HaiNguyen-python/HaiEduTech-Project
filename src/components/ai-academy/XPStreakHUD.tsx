...
const XPStreakHUD = () => {
  ...
  const handleClaim = () => {
    ...
    if (granted > 0) {
      ...
      toast({
        title: `🎁 +${granted} XP nhiệm vụ ngày!`,
        description: "Quay lại ngày mai để giữ chuỗi học liên tiếp 🔥",
      });
    }
  };
    ...
    >
      ...
      <div className="rounded-2xl border-2 border-purple-300/40 dark:border-purple-500/30 bg-gradient-to-br from-purple-500/15 via-fuchsia-500/10 to-indigo-500/15 backdrop-blur p-3">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Cấp {current.level}</div>
        ...
        <div className="text-[10px] text-muted-foreground mt-1 text-right">
          {next ? `${next.xpRequired - state.xp} XP → ${next.emoji} ${next.name}` : "Bạn đã đạt cấp tối đa! 👑"}
        </div>
      </div>
      ...
      <div className="rounded-2xl border-2 border-orange-300/50 dark:border-orange-500/30 bg-gradient-to-br from-orange-500/15 via-red-500/10 to-amber-500/15 backdrop-blur p-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold flex items-center gap-1">
            <Flame className="w-3 h-3" /> Chuỗi ngày học
          </div>
          <div className="text-2xl font-black text-orange-600 dark:text-orange-300">
            {state.streak} <span className="text-sm font-bold">ngày</span>
          </div>
          <div className="text-[10px] text-muted-foreground">
            {state.streak === 0 ? "Học hôm nay để bắt đầu chuỗi!" : "Quay lại mỗi ngày để không mất chuỗi 💪"}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border-2 border-emerald-300/50 dark:border-emerald-500/30 bg-gradient-to-br from-emerald-500/15 via-teal-500/10 to-cyan-500/15 backdrop-blur p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold flex items-center gap-1">
            <Trophy className="w-3 h-3" /> Nhiệm vụ hôm nay
          </div>
        ...
        <div className="space-y-1 mb-2">
          <QuestItem done={quest.lessonDone} label="Mở 1 bài học" />
          <QuestItem done={quest.quizDone} label="Hoàn thành 1 quiz" />
          <QuestItem done={quest.starDone} label="Kiếm 1 sao ⭐" />
        </div>
        {canClaim ? (
          <Button
            size="sm"
          ...
          >
            <Gift className="w-3 h-3 mr-1" /> Nhận +50 XP
          </Button>
        ) : quest.rewardClaimed ? (
