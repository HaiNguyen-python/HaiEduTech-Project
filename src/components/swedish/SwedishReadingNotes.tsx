 ...
 *              glossary (English meanings). Both panels are collapsible so the
 *              page stays compact. Presentation only - analysis lives in
 *              swedishReadingNotes.
 * @author Teacher Hai (HaiEduTech)
 * @copyright 2026 HaiEduTech, ILC. All rights reserved.
 */
import { useMemo, useState } from "react";
import { GraduationCap, BookMarked, ChevronDown } from "lucide-react";
...
const SwedishReadingNotes = ({ textSv, keyVocab }: Props) => {
  ...
  return (
    <div className="mb-6 grid gap-4 lg:grid-cols-2">
      {grammar.length > 0 && (
        <Collapsible open={openGrammar} onOpenChange={setOpenGrammar}>
      ...
      {words.length > 0 && (
        <Collapsible open={openWords} onOpenChange={setOpenWords}>
