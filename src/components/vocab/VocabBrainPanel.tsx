  ...
  return (
    <section className="mt-10 rounded-2xl border border-border bg-card/60 p-4 sm:p-6">
      ...
      {totalMastered > 0 && !loading && (
        <div className="mb-2 flex flex-wrap items-center gap-1.5 rounded-xl border border-border bg-muted/40 p-2">
          {FILTERS.map(f => (
            <Button
              key={f.key}
              size="sm"
          ...
          <Button
            ...
            title={showLabels ? t("Đang hiện chữ", "Labels on") : t("Chỉ chấm sáng", "Dots only")}
            onClick={() => setShowLabels(v => !v)}
          >
            {showLabels ? <Type className="h-3.5 w-3.5" /> : <Sparkles className="h-3.5 w-3.5" />}
          </Button>
          {showLabels && (
            <div className="flex items-center gap-1 rounded-lg bg-background/70 p-0.5">
              {DENSITIES.map(d => (
                <button
                  key={d.key}
          ...
          <Button
            ...
            variant="secondary"
            className="h-8 w-8 p-0"
            title={paused ? t("Xoay tiếp", "Rotate") : t("Tạm dừng", "Pause")}
          ...
          <Button
            ...
            variant="secondary"
            className="h-8 w-8 p-0"
            title={replay === null ? t("Xem quá trình", "Replay consolidation") : t("Đang chạy...", "Playing...")}
            onClick={() => setReplay(0)}
          ...
          <Button
            ...
            variant="secondary"
            className="h-8 w-8 p-0"
            title={t("Góc nhìn gốc", "Reset view")}
            onClick={() => setViewKey(k => k + 1)}
          >
            <Crosshair className="h-3.5 w-3.5" />
          </Button>

          {/* Search: an icon button that expands only when needed */}
          {searchOpen || query ? (
