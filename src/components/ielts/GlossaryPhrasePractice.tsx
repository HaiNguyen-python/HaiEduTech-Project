 ...
 * @description Inline per-glossary-row writing practice. An example sentence is
 *   shown directly (from the glossary `context`) and the student rewrites it
 *   below to practice using the phrase. The existing `grade-phrase-sentence`
 *   Perplexity-powered edge function scores grammar + correct phrase usage and
 *   returns an upgraded Band 7.5+ version.
 */
import { useState } from "react";
import { motion } from "framer-motion";
import { Loader2, CheckCircle2, AlertCircle, Lightbulb, Wand2, Sparkles, PenLine } from "lucide-react";
    ...
    <div className="mt-2 rounded-lg border bg-muted/20 p-2.5 space-y-2">
      ...
      {result && (
        <motion.div
          ...
          className="space-y-2 rounded-md bg-background border p-2.5"
        >
