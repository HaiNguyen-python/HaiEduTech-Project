...
export default function SmartResourcesContent() {
  const { t } = useLanguage();
    ...
    try {
      ...
      if (user) {
        await supabase.from("learning_materials").insert({
          language, level, material_type: materialType,
          title: data.title, content: data,
          question_count: questionCount, created_by: user.id,
        });
      }
    } catch (err: any) {
      toast({ title: "Lỗi tạo bài tập", description: err.message || "Không thể tạo nội dung.", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitQuiz = () => {
    ...
    toast({
      ...
      description: correct === content.questions.length ? "Xuất sắc! 🎉" : "Xem lại giải thích để học thêm nhé!",
    });
  };

  const handleDownloadPDF = () => {
    if (!content) return;
    const html = `<html><head><meta charset="utf-8"><title>${content.title}</title>
      ...
      .correct{color:#38a169;font-weight:bold}
      .explanation{margin-top:8px;padding:12px;background:#f0fff4;border-radius:6px;font-size:0.9em}</style></head>
      ...
      ${content.questions.map((q, i) => `
        ...
        ${q.options.map((o, j) => `<div class="${j === q.correct ? "correct" : ""}">${String.fromCharCode(65 + j)}. ${o}</div>`).join("")}
        <div class="explanation"><strong>Giải thích:</strong> ${q.explanation}</div></div>
      `).join("")}</body></html>`;
    const blob = new Blob([html], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `${content.title.replace(/\s+/g, "_")}.html`; a.click();
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <Card className="max-w-3xl mx-auto mb-10 shadow-md border-border/60">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                {MATERIAL_TYPES.map((mt) => {
                  ...
                  return (
                    <button key={mt.value} onClick={() => setMaterialType(mt.value)}
                      className={`flex flex-col items-center gap-2 p-4 rounded-lg border-2 transition-all duration-200 active:scale-[0.97] ${isActive ? "border-primary bg-primary/5 text-primary shadow-sm" : "border-border hover:border-primary/40 text-muted-foreground hover:text-foreground"}`}>
              ...
              <div className="flex gap-2">
                {QUESTION_COUNTS.map((n) => (
                  <button key={n} onClick={() => setQuestionCount(n)}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 active:scale-95 ${questionCount === n ? "bg-primary text-primary-foreground shadow-sm" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
                    {n} {t("câu", "Q")}
                  </button>
                ))}
              </div>
            ...
            <Button onClick={handleGenerate} disabled={loading || !language || !level} className="w-full h-12 text-base font-semibold" size="lg">
              {loading ? (<><Loader2 className="w-5 h-5 animate-spin" />{t("Đang tạo...", "Generating...")}</>) : (<><Sparkles className="w-5 h-5" />{t("Tạo bài tập", "Generate Exercise")}</>)}
            </Button>
          </CardContent>
        </Card>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {content && (
          <motion.div key="content" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.5 }} className="max-w-3xl mx-auto">
            <Card className="shadow-lg border-border/60">
              <CardHeader className="flex flex-row items-start justify-between gap-4 pb-3">
                ...
                <Button variant="outline" size="sm" onClick={handleDownloadPDF}>
                  <Download className="w-4 h-4 mr-1" />{t("Tải xuống", "Download")}
                </Button>
              </CardHeader>
              <CardContent className="space-y-6">
                {content.passage && (
                  <div className="p-5 bg-muted/50 rounded-lg border border-border/50 leading-relaxed text-foreground/90 whitespace-pre-wrap">{content.passage}</div>
                )}
                <div className="space-y-5">
                  {content.questions.map((q, qi) => (
                    <motion.div key={qi} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: qi * 0.08 }}
                      className="p-4 rounded-lg border border-border/50 bg-card">
                      <p className="font-medium mb-3"><span className="text-primary font-bold mr-2">Câu {qi + 1}.</span>{q.question}</p>
                      <RadioGroup value={userAnswers[qi]?.toString()} onValueChange={(v) => setUserAnswers((prev) => ({ ...prev, [qi]: parseInt(v) }))} disabled={showResults} className="space-y-2">
                        {q.options.map((opt, oi) => {
                          ...
                          return (
                            <label key={oi} className={`flex items-center gap-3 p-3 rounded-md border cursor-pointer transition-colors ${optClass} ${!showResults && isSelected ? "border-primary bg-primary/5" : !showResults ? "hover:bg-muted/50" : ""}`}>
                              <RadioGroupItem value={oi.toString()} />
                              <span className="text-sm flex-1"><span className="font-medium mr-1.5">{String.fromCharCode(65 + oi)}.</span>{opt}</span>
                ...
                <div className="flex gap-3 pt-2">
                  {!showResults ? (
                    <Button onClick={handleSubmitQuiz} disabled={Object.keys(userAnswers).length < content.questions.length} className="flex-1">{t("Nộp bài", "Submit")}</Button>
                  ) : (
                    <Button onClick={() => { setUserAnswers({}); setShowResults(false); }} variant="outline" className="flex-1">{t("Làm lại", "Retry")}</Button>
                  )}
                  <Button variant="outline" onClick={handleGenerate} disabled={loading}>
                    <Sparkles className="w-4 h-4 mr-1" />{t("Tạo bài mới", "New Exercise")}
                  </Button>
                </div>
              </CardContent>
            </Card>
