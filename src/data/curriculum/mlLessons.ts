...
**Multi-class:** One-vs-Rest, One-vs-One, or Softmax Regression (probabilities sum to 1).`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Vì sao PayPal/Stripe vẫn dùng Logistic Regression làm first-stage filter cho fraud detection dù đã có deep learning?", options: ["Vì model phức tạp không hoạt động", "Vì latency <5ms cho 99% giao dịch hợp lệ, sau đó mới chuyển case nghi ngờ qua model XGBoost phức tạp hơn", "Vì rẻ tiền", "Vì lập trình viên cũ"], answer: 1, explanation: "Hybrid architecture: LogReg lọc nhanh hàng triệu giao dịch/giây (high recall, threshold thấp), chỉ ~1% nghi ngờ được chuyển qua model deep learning. Tiết kiệm 95% chi phí inference mà vẫn đảm bảo SLA." },
          { question: "🏢 Knowledge Check: COMPAS recidivism algorithm (ProPublica 2016) bị chỉ trích vì lý do gì liên quan đến Logistic Regression?", options: ["Quá chậm", "Black defendants có FPR (False Positive Rate) gấp 2x white defendants — bias trong training data tạo ra unfair predictions", "Accuracy quá thấp", "Không scale được"], answer: 1, explanation: "Dù COMPAS đạt overall accuracy ~65%, nhưng error distribution không công bằng giữa các nhóm chủng tộc. Đây là minh chứng kinh điển cho 'fairness vs accuracy trade-off' trong ML — calibration tốt không đảm bảo fairness." },
          { question: "🏢 Knowledge Check: Trong y khoa, vì sao chẩn đoán ung thư ưu tiên Recall hơn Precision dù gây nhiều False Positive?", options: ["Để tiết kiệm chi phí", "Vì miss một ca ung thư (FN) gây tử vong, trong khi FP chỉ dẫn đến biopsy thêm — chi phí FN >> FP", "Vì model dễ train hơn", "Vì luật bắt buộc"], answer: 1, explanation: "Cost matrix asymmetric: FN = mất mạng người (vô giá), FP = thêm biopsy (vài nghìn USD). Threshold được hạ xuống thấp để recall ~99%, chấp nhận precision thấp hơn. Đây là core principle của medical AI." }
        ]
      }
    ]
  },
...
**Pros:** Interpretable, no scaling, handles mixed types, fast inference. **Cons:** Overfits, unstable, greedy, high variance.`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: IBM Watson for Oncology ($62M failure, 2018) — vì sao Decision Trees + ensemble không chẩn đoán ung thư chính xác?", options: ["Vì model quá đơn giản", "Vì training data là synthetic cases do bác sĩ MSKCC tạo ra (không phải real patients), dẫn đến overfit + bias địa lý", "Vì không đủ tiền", "Vì hardware yếu"], answer: 1, explanation: "Watson được train trên hypothetical cases thay vì real-world patient outcomes. Model học được pattern của bác sĩ MSKCC chứ không phải biology của ung thư — thất bại nặng khi deploy ở bệnh viện khác (Hàn Quốc, Đức)." },
          { question: "🏢 Knowledge Check: Vì sao ngân hàng (Capital One, JPMorgan) chuộng Decision Trees đơn lẻ thay vì Neural Networks cho credit scoring?", options: ["Vì rẻ hơn", "Vì interpretability bắt buộc theo luật ECOA/FCRA — phải giải thích được lý do từ chối khoản vay với khách hàng", "Vì accuracy cao hơn", "Vì nhanh hơn"], answer: 1, explanation: "Equal Credit Opportunity Act yêu cầu 'adverse action notice' — giải thích cụ thể vì sao từ chối. Decision tree path ('Income < $50K AND Debt > 40%') dễ explain. Neural network = black box, vi phạm luật." },
          { question: "🏢 Knowledge Check: Trong Kaggle competitions, vì sao single Decision Tree hiếm khi thắng dù được dùng làm baseline?", options: ["Vì nó quá phức tạp", "Vì high variance (overfit nặng) — chỉ thay đổi nhỏ ở data tạo ra tree hoàn toàn khác. Cần ensemble (RF, XGBoost) để giảm variance", "Vì nó chỉ làm classification", "Vì nó không hỗ trợ GPU"], answer: 1, explanation: "Single tree có bias-variance trade-off cực tệ: depth thấp → underfit, depth cao → overfit. Kaggle winners luôn dùng ensemble (XGBoost, LightGBM) vì averaging giảm variance mà vẫn giữ low bias." }
        ]
      }
    ]
  },
...
**Pros:** Accurate, hard to overfit, parallelizable. **Cons:** Less interpretable, slower, memory-heavy, can't extrapolate.`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Vì sao Microsoft Kinect (2010) dùng Random Forest thay vì Neural Networks cho real-time body pose estimation?", options: ["Vì NN chưa tồn tại", "Vì RF inference cực nhanh trên CPU (1ms/frame), parallel hóa tốt — đáp ứng 30fps trên Xbox 360 không có GPU mạnh", "Vì RF chính xác hơn", "Vì code dễ viết hơn"], answer: 1, explanation: "Năm 2010 deep learning chưa phổ biến, Xbox 360 không có GPU. Random Forest với mỗi tree độc lập có thể parallelize trên multi-core CPU. Inference time predictable — critical cho real-time gaming." },
          { question: "🏢 Knowledge Check: Trong Kaggle 2014-2018, vì sao Random Forest dần bị XGBoost soán ngôi dù cả hai đều ensemble?", options: ["RF quá chậm", "Boosting (XGBoost) sửa lỗi tuần tự nên bias thấp hơn bagging (RF). XGBoost còn có regularization L1/L2 + handle missing values native", "RF không hỗ trợ Python", "RF chỉ dùng được cho classification"], answer: 1, explanation: "RF giảm variance nhưng không giảm bias. XGBoost giảm cả variance lẫn bias nhờ sequential boosting + gradient optimization. Trên tabular data, XGBoost thường win 0.5-2% so với RF — đủ để thắng Kaggle." },
          { question: "🏢 Knowledge Check: Tại Airbnb, Random Forest được dùng cho 'Smart Pricing' — feature nào quan trọng nhất khi inspect feature_importances_?", options: ["Tên host", "Location (lat/lng) + property_type + seasonality — RF tự động phát hiện non-linear interactions", "Màu ảnh listing", "Số ký tự trong description"], answer: 1, explanation: "RF feature importance dựa trên mean decrease in impurity. Airbnb's pricing model phát hiện location-seasonality interaction (vd: ski resort đắt mùa đông) tự động — không cần feature engineering thủ công." }
        ]
      }
    ]
  },
...
**Best for:** Medium datasets, complex boundaries, high-dimensional data. **Not ideal for:** Large datasets (O(n²-n³)).`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Vì sao Gmail Spam Filter (2004-2012) dùng SVM với linear kernel thay vì RBF?", options: ["Vì RBF không hoạt động", "Vì text data có hàng triệu features (bag-of-words) — linear kernel đủ tốt và scale tới hàng tỷ emails, RBF quá chậm O(n²)", "Vì linear dễ code hơn", "Vì RBF tốn RAM"], answer: 1, explanation: "Trong high-dimensional sparse text (50K-1M features), data thường đã linearly separable — RBF không cải thiện gì. Linear SVM (LIBLINEAR) train trong O(n) thay vì O(n²), xử lý được scale của Gmail." },
          { question: "🏢 Knowledge Check: Vì sao SVM bị deep learning thay thế trong computer vision sau 2012 (AlexNet)?", options: ["SVM quá đắt", "Kernel trick không scale với millions of images, và CNN tự học features tốt hơn handcrafted features (HOG, SIFT) + SVM", "SVM không hỗ trợ ảnh màu", "Bản quyền"], answer: 1, explanation: "Pre-2012: HOG/SIFT features + SVM là SOTA cho ImageNet. AlexNet 2012 giảm error 26%→16% bằng end-to-end CNN. SVM với precomputed kernel matrix tốn O(n²) memory, không scale với 1M+ images." },
          { question: "🏢 Knowledge Check: Trong bioinformatics (cancer gene classification), vì sao SVM vẫn dominant đến nay?", options: ["Vì tradition", "Vì high-dimensional small-sample (10K genes, 100 patients) — SVM với kernel handle 'curse of dimensionality' tốt hơn deep learning cần big data", "Vì rẻ tiền", "Vì luật bắt buộc"], answer: 1, explanation: "Khi p (features) >> n (samples), deep learning overfit nặng. SVM maximizes margin → robust với high-dimensional. Kernel cho phép modeling non-linear mà không cần thêm samples. RBF/string kernels phổ biến trong genomics." }
        ]
      }
    ]
  },
...
**Applications:** Customer segmentation, image compression, document clustering, anomaly detection, feature engineering.`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Spotify Discover Weekly dùng K-Means cho mục đích gì trước khi sang collaborative filtering?", options: ["Phân loại bài hát theo genre cố định", "Cluster users thành 'taste profiles' (vd: indie-folk lover, late-night techno) — pre-compute giúp giảm 100x cost của collaborative filtering", "Tạo playlist ngẫu nhiên", "Lọc spam"], answer: 1, explanation: "Với 500M users, tính similarity từng cặp (collaborative filtering) là O(n²). Spotify cluster users thành ~10K taste profiles trước, sau đó chỉ tính similarity trong cluster — giảm cost từ tỷ ops xuống triệu ops." },
          { question: "🏢 Knowledge Check: Vì sao Customer Segmentation tại Starbucks dùng K-Means thay vì DBSCAN dù DBSCAN handle non-spherical clusters tốt hơn?", options: ["Vì DBSCAN quá mới", "Vì K-Means cho fixed K interpretable cho marketing team (vd: 5 segments rõ ràng), DBSCAN tạo số cluster không kiểm soát được + outliers khó action", "Vì DBSCAN không scale", "Vì K-Means free"], answer: 1, explanation: "Marketing cần actionable segments với business meaning ('Gold Members', 'Weekend Warriors'). K=5 cố định, mỗi cluster có centroid → persona rõ ràng. DBSCAN có thể tạo 50 clusters + 30% noise points → không thể design campaign." },
          { question: "🏢 Knowledge Check: Trong image compression (color quantization), K-Means dùng như thế nào và limitation chính là gì?", options: ["Không liên quan đến ảnh", "Cluster các pixels theo RGB thành K màu đại diện (palette). Limitation: K cố định, không adapt với regions phức tạp — gây posterization artifact", "Dùng để encode JPEG", "Tạo ảnh blur"], answer: 1, explanation: "K-Means giảm 16M màu xuống K màu (vd: K=256 cho GIF). Mỗi pixel được assign màu gần nhất trong palette. Vùng gradient mịn (sky) bị banding vì K màu không đủ — cần Floyd-Steinberg dithering bù lại." }
        ]
      }
    ]
  },
...
**Pipeline:** Handle Missing → Encode → Create → Scale → Select → Model.`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Vì sao Amazon's hiring AI (2018) bị scrap dù feature engineering rất kỹ lưỡng?", options: ["Vì model quá đơn giản", "Vì training data 10 năm CV chủ yếu nam giới → model học cách penalize từ 'women's' (women's chess club) — gender bias từ historical data", "Vì code lỗi", "Vì AWS quá đắt"], answer: 1, explanation: "Amazon train trên 10 năm CV của Amazon employees (đa số nam). Model phát hiện pattern 'nam → hire' và sử dụng proxy features (verbs, school names) để penalize nữ. Đây là kinh điển: fairness bug từ feature engineering không thể fix bằng debias hậu kỳ." },
          { question: "🏢 Knowledge Check: Trong Kaggle Home Credit competition (2018), feature engineering nào quan trọng nhất giúp top 1%?", options: ["Random features", "Aggregation features từ bureau data (max, mean, std của previous loans) + ratio features (debt-to-income) — chiếm 60% feature importance", "Drop tất cả features", "One-hot encoding tên"], answer: 1, explanation: "Top winners spend 80% time vào feature engineering hơn là model tuning. Aggregation từ relational tables + domain-specific ratios (DTI, payment_to_income) đánh bại neural networks trên tabular data — đây là lý do XGBoost vẫn dominate finance." },
          { question: "🏢 Knowledge Check: Target leakage trong medical ML — ví dụ cụ thể từ paper 'Reproducible ML' (Kapoor & Narayanan 2023) là gì?", options: ["Dùng RAM quá nhiều", "Sử dụng feature 'patient transferred to ICU' để predict mortality — feature này chỉ có sau khi outcome đã xác định, gây AUC giả 0.99 nhưng useless trong production", "Forget to log", "Wrong learning rate"], answer: 1, explanation: "Khoảng 294 ML papers trong COVID-19 detection bị phát hiện có target leakage: dùng features được record SAU khi diagnosis. Models đạt AUC 0.99 trong validation nhưng thất bại hoàn toàn trên prospective data — vấn đề temporal leakage." }
        ]
      }
    ]
  },
...
**Common Mistakes:** Data leakage (preprocess before split), evaluating on training data, using test set for tuning, not stratifying, shuffling time series, ignoring groups.`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Vì sao Renaissance Technologies (Medallion Fund, 66% return/year) dùng walk-forward CV thay vì K-Fold?", options: ["Vì K-Fold quá chậm", "Vì financial data có temporal autocorrelation và regime changes — walk-forward simulate chính xác trading reality, K-Fold tạo data leakage qua time", "Vì truyền thống", "Vì tiết kiệm RAM"], answer: 1, explanation: "Trading models trade dựa trên past data only. K-Fold cho phép model 'thấy' future patterns trong fold khác → backtest đẹp nhưng live trading lỗ. Walk-forward (expanding window) mô phỏng đúng quy trình: train past → test next period → roll forward." },
          { question: "🏢 Knowledge Check: Stratified K-Fold quan trọng nhất khi nào, theo case study Kaggle Credit Card Fraud (492 frauds / 284,807 transactions)?", options: ["Khi data balanced", "Khi imbalanced cực đoan (0.17% positive) — random split có thể tạo fold không có fraud nào, làm metric không tin cậy", "Khi data nhỏ", "Khi nhiều features"], answer: 1, explanation: "Với 0.17% positive class, random K-Fold (K=10) có thể tạo fold với 0-3 frauds — variance của F1 score cực cao. Stratified K-Fold đảm bảo mỗi fold có tỷ lệ class giống nhau, giảm CV variance từ ±0.15 xuống ±0.02." },
          { question: "🏢 Knowledge Check: Nested Cross-Validation tốn 100x compute — khi nào thực sự cần thiết?", options: ["Luôn cần", "Khi cần unbiased estimate của model performance VÀ tuning hyperparameters đồng thời (vd: medical research, regulatory submissions)", "Khi data lớn", "Khi nhanh"], answer: 1, explanation: "Standard K-Fold tuning leak hyperparameter selection bias vào CV score. Nested CV (outer loop: estimate, inner loop: tune) cho unbiased estimate. Bắt buộc cho FDA submissions, academic papers — không cần cho prototyping." }
        ]
      }
    ]
  },
...
**Best Practice:** Random → narrow with Grid/Bayesian. Use CV. Log experiments. Tune impactful params first. Use log-scale for learning rates.`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: OpenAI GPT-3 ($4.6M training run) — họ tune hyperparameters ra sao trước khi commit full training?", options: ["Trial and error trên full model", "Train hàng nghìn small models (125M params) để tìm optimal lr, batch size, sau đó scale up theo Chinchilla scaling laws (compute-optimal)", "Random search trên 175B model", "Copy từ paper khác"], answer: 1, explanation: "Full training $4.6M không cho phép trial-and-error. OpenAI tune trên proxy models (1000x nhỏ hơn) rồi extrapolate hyperparameters theo power law. Chinchilla (DeepMind 2022) chứng minh GPT-3 thực ra under-trained — optimal là 4x ít params + 4x more data." },
          { question: "🏢 Knowledge Check: Vì sao Google Vizier (Bayesian Optimization) tiết kiệm 10x compute so với Grid Search trong production tuning?", options: ["Vì Bayesian rẻ hơn", "Vì Vizier dùng Gaussian Process học từ trials trước, focus vào promising regions thay vì exhaustive search — phù hợp với expensive objective (training neural networks)", "Vì Google có TPU", "Vì code optimized"], answer: 1, explanation: "Grid search waste compute trên bad regions. Vizier model objective function bằng GP, dùng acquisition function (UCB, EI) để chọn next trial. Tại Google, Vizier tune AutoML, dialog systems — 10-100x sample efficient hơn random/grid search." },
          { question: "🏢 Knowledge Check: Hyperparameter tuning có 'reverse importance' problem — ví dụ từ Bergstra & Bengio (2012) là gì?", options: ["Tất cả params đều quan trọng", "Chỉ ~3 hyperparameters thực sự matter (lr, batch size, regularization) — grid search waste 90% compute trên unimportant params, random search hiệu quả hơn", "Cần tune tất cả 50+ params", "Importance ngẫu nhiên"], answer: 1, explanation: "Bergstra paper landmark: 9-D grid search với 3^9=19683 trials chỉ tốt hơn random search với 27 trials một chút. Vì chỉ vài params dominant, random search cover effective space tốt hơn. Đây là lý do Random Search được khuyến nghị làm baseline." }
        ]
      }
    ]
  },
...
**Regression metrics:** MSE (penalizes large errors), RMSE (same units), MAE (robust), MAPE (relative), R² (% explained).`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Google Diabetic Retinopathy AI (2018) đạt AUC 0.97 trong lab nhưng thất bại tại Thailand clinics — vì sao?", options: ["Model bị hack", "Lab images chất lượng cao (well-lit, focused), thực tế clinic Thailand có ảnh mờ/thiếu sáng — model reject 21% cases làm bệnh nhân phải đi khám lại, lost trust", "Server quá chậm", "Hết tiền"], answer: 1, explanation: "Đây là kinh điển 'evaluation metric ≠ deployment success'. AUC chỉ đo discriminative power, không capture: image quality variance, workflow integration, patient experience. Bài học: cần metrics multi-dimensional (utility, fairness, robustness)." },
          { question: "🏢 Knowledge Check: Tại Spotify, vì sao họ KHÔNG dùng accuracy để evaluate Discover Weekly recommendations?", options: ["Accuracy là metric tốt nhất", "Vì 'click-through rate' và 'long listen rate (>30s)' phản ánh đúng business value hơn — accuracy trên implicit feedback là meaningless", "Vì Spotify dùng SQL", "Vì luật bắt buộc"], answer: 1, explanation: "Recommendation systems không có ground truth 'đúng/sai'. Spotify dùng nested metrics: CTR (engagement), 30s listen (intent), saves (preference), repeat plays (loyalty). Mỗi metric tracking different business goal — A/B test trên multiple metrics song song." },
          { question: "🏢 Knowledge Check: Bias-Variance trade-off trong production: vì sao Netflix prefer high-bias models cho new users (cold start)?", options: ["Vì high variance tốt hơn", "Vì new users có ít data (n<10), high-variance models overfit nặng — high-bias models (popularity-based) generalize tốt hơn cho cold start", "Vì model rẻ hơn", "Vì Netflix tiết kiệm"], answer: 1, explanation: "Cold start = small n. Với n=5 ratings, complex model (deep CF) có variance cực cao, predict random. Simple model (popular trong same demographic) bias cao nhưng variance thấp → tốt hơn. Netflix gradually shift sang complex model khi user data tăng." }
        ]
      }
    ]
  },
...
**Practical:** RF (baseline) → XGBoost/LightGBM (best) → Stacking (marginal gains).`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Vì sao XGBoost thắng 60% Kaggle competitions từ 2015-2018 cho tabular data?", options: ["Vì XGBoost free", "Vì combine: gradient boosting + L1/L2 regularization + handle missing values native + tree pruning + parallel compute — sweet spot accuracy/speed cho tabular", "Vì trend", "Vì dễ học"], answer: 1, explanation: "Tianqi Chen (XGBoost creator) tối ưu 5 yếu tố: (1) regularized objective tránh overfit, (2) sparse-aware split cho missing values, (3) weighted quantile sketch giảm memory, (4) cache-aware prefetching, (5) out-of-core computation. Đây là engineering masterpiece." },
          { question: "🏢 Knowledge Check: BloombergGPT (2023, $2.7M training) dùng ensemble như thế nào trong financial sentiment analysis?", options: ["Single model duy nhất", "Stack BloombergGPT + FinBERT + GPT-4 với meta-learner — mỗi model bắt khía cạnh khác (domain knowledge, general reasoning), reduce variance trên rare events", "Random ensemble", "Không dùng ensemble"], answer: 1, explanation: "Financial events (earnings surprise, fed decisions) là rare và high-stakes. Single model fail unpredictably. Stacking với diverse base models (domain-specific + general) tạo robust predictions. Trade-off: 3x inference cost, nhưng mỗi false signal có thể tốn $millions." },
          { question: "🏢 Knowledge Check: Netflix Prize winners blend 100+ models — vì sao họ KHÔNG deploy ensemble cuối cùng vào production?", options: ["Vì model thắng quá chậm", "Vì ensemble inference quá phức tạp (100+ models, blend layer) cho 100M users real-time. Netflix chỉ cherry-pick 2-3 ý tưởng từ ensemble winner để integrate", "Vì hết hợp đồng", "Vì quá đắt license"], answer: 1, explanation: "Engineering trade-off kinh điển: research win ≠ production-ready. 10% RMSE improvement không đáng nếu latency tăng 100x. Netflix học được: matrix factorization + temporal dynamics, deploy individually thay vì full ensemble. Bài học cho mọi MLOps team." }
        ]
      }
    ]
  },
...
**Best Practices:** Version everything, A/B testing, shadow mode, canary deployment, rollback plan, model cards, alert on everything.`,
        ...
        quiz: [
          ...
          { question: "🏢 Knowledge Check: Vì sao Uber Michelangelo (MLOps platform) tách 'feature store' khỏi 'model serving'?", options: ["Vì code dễ hơn", "Vì cùng feature engineering logic phải dùng nhất quán giữa training và serving — nếu khác nhau gây 'training-serving skew' (model accuracy giảm 10-30% trong production)", "Vì AWS bắt buộc", "Vì security"], answer: 1, explanation: "Training-serving skew là vấn đề #1 trong MLOps. Nếu training tính 'avg_purchase_30d' bằng SQL batch, serving tính bằng Python streaming — nhỏ khác biệt (timezone, null handling) gây drift. Feature Store (Tecton, Feast) đảm bảo single source of truth cho features." },
          { question: "🏢 Knowledge Check: Google's 'Hidden Technical Debt in ML Systems' (2015) chỉ ra ML code chỉ chiếm bao nhiêu % codebase production?", options: ["80%", "Chỉ ~5% — phần còn lại là data pipelines, monitoring, serving infra, configuration, glue code. Đây là lý do MLOps quan trọng hơn ML thuần", "50%", "100%"], answer: 1, explanation: "Sculley et al. paper landmark: ML code (model.fit) chỉ là tip of the iceberg. 95% effort là engineering: data validation, feature pipelines, A/B testing infra, monitoring, rollback systems. Bài học: hire ML engineers (full-stack) hơn pure data scientists cho production teams." },
          { question: "🏢 Knowledge Check: Tại sao Stripe giữ TWO models chạy song song (champion + challenger) cho fraud detection?", options: ["Để đốt tiền", "Champion serve 99% traffic (proven), challenger serve 1% (new model). Compare metrics weekly, promote challenger nếu beat champion 2 tuần liên tục — safe rollout với rollback nhanh", "Vì Stripe rich", "Vì luật"], answer: 1, explanation: "Champion-Challenger pattern là gold standard cho high-stakes ML. Stripe không bao giờ replace model toàn phần — luôn có canary deployment. Nếu challenger gây false positive spike (block legit transactions), traffic shift về champion trong vài giây." },
          { question: "🏢 Knowledge Check: Concept drift vs Data drift — vd cụ thể từ Zillow Offers ($304M loss 2021) là gì?", options: ["Cùng một thứ", "Data drift: house prices distribution shift (post-COVID inflation). Concept drift: relationship giữa features và price thay đổi (work-from-home → suburban premium). Cả hai cùng xảy ra → model deprecate trong tuần", "Chỉ data drift", "Chỉ concept drift"], answer: 1, explanation: "Zillow's Zestimate train trên pre-COVID era. Sau COVID: (1) data drift — giá tăng 20% (input distribution shift), (2) concept drift — buyer preferences đổi (suburban > urban, larger homes). Single model không thể adapt → đến khi phát hiện đã mua hàng nghìn nhà giá quá cao." }
        ]
      }
    ]
  }
