...
# response = llm.chat(messages=build_prompt(turn), temperature=0.4, max_tokens=200)',
        ...
        quiz: [
          { question: "Vì sao chỉ giữ 6 turn gần nhất trong context?", options: ["Trang trí", "Giảm token + nhiễu, vẫn đủ ngữ cảnh ngắn hạn", "Bắt buộc bởi OpenAI", "Tăng độ trễ"], answer: 1, explanation: "Quá dài → tốn tiền và 'lost-in-the-middle'.", questionEn: "Why keep only the last 6 turns in context?", optionsEn: ["Decoration", "Reduces tokens + noise while keeping enough short-term context", "Required by OpenAI", "Increases latency"], explanationEn: "Too long → wastes money and causes 'lost-in-the-middle'." },
          { question: "Temperature nào hợp cho chấm điểm tự động?", options: ["1.5", "1.0", "0.2", "Random"], answer: 2, explanation: "Cần ổn định, deterministic.", questionEn: "Which temperature suits automated grading?", optionsEn: ["1.5", "1.0", "0.2", "Random"], explanationEn: "Grading needs stability and determinism." },
          { question: "Sai lầm KHÔNG nên có?", options: ["Log conversation", "Giới hạn độ dài reply", "Nhồi 50KB giáo trình mỗi prompt", "Có evaluator"], answer: 2, explanation: "RAG đúng cách chỉ inject 2-4 passages liên quan.", questionEn: "Which mistake should be avoided?", optionsEn: ["Logging conversations", "Limiting reply length", "Stuffing a 50 KB textbook into every prompt", "Having an evaluator"], explanationEn: "Proper RAG only injects 2-4 relevant passages." },
          { question: "Socratic tutor nên?", options: ["Cho đáp án ngay", "Đặt câu hỏi dẫn dắt", "Im lặng", "Báo lỗi"], answer: 1, explanation: "Học sinh tự nghĩ ra → nhớ lâu hơn.", questionEn: "What should a Socratic tutor do?", optionsEn: ["Give the answer immediately", "Ask guiding questions", "Stay silent", "Report an error"], explanationEn: "Students who figure it out themselves remember longer." },
          { question: "Component nào bắt buộc để cá nhân hóa?", options: ["Logo", "Memory (profile + mastery)", "Dark mode", "Pricing page"], answer: 1, explanation: "Không có memory = chatbot generic.", questionEn: "Which component is required for personalization?", optionsEn: ["Logo", "Memory (profile + mastery)", "Dark mode", "Pricing page"], explanationEn: "Without memory it's just a generic chatbot." },
        ],
      },
      {
        id: "edtech-ai-2",
...
def rag_answer(conn, question: str) -> str:
    ...
    return client.chat.completions.create(
        ...
        quiz: [
          { question: "Chunk overlap để làm gì?", options: ["Tăng cost", "Tránh mất ngữ cảnh ở biên chunk", "Decoration", "Required by law"], answer: 1, explanation: "Câu nằm giữa 2 chunk vẫn được retrieve.", questionEn: "What is chunk overlap for?", optionsEn: ["Increasing cost", "Avoiding lost context at chunk boundaries", "Decoration", "Required by law"], explanationEn: "A sentence spanning two chunks can still be retrieved." },
          { question: "Khi nào dùng hybrid retrieval?", options: ["Không bao giờ", "Khi có nhiều danh từ riêng / code", "Khi không có embed model", "Khi data nhỏ"], answer: 1, explanation: "BM25 bắt token chính xác mà vector hay miss.", questionEn: "When should you use hybrid retrieval?", optionsEn: ["Never", "When there are many proper nouns / code tokens", "When there is no embedding model", "When the data is small"], explanationEn: "BM25 catches exact tokens that vector search often misses." },
          { question: "Faithfulness đo gì?", options: ["Tốc độ", "Answer có dựa trên chunks không (không bịa)", "Số chunk", "Latency"], answer: 1, explanation: "Là tuyến phòng thủ chống hallucination.", questionEn: "What does faithfulness measure?", optionsEn: ["Speed", "Whether the answer is supported by the chunks (no fabrication)", "Number of chunks", "Latency"], explanationEn: "It's a defense line against hallucination." },
          { question: "k quá lớn (k=20) gây gì?", options: ["Tăng faithfulness", "Tốn token + 'lost-in-the-middle'", "Tăng recall vô hạn", "Không vấn đề"], answer: 1, explanation: "LLM bỏ sót thông tin ở giữa context dài.", questionEn: "What happens if k is too large (k=20)?", optionsEn: ["Increases faithfulness", "Wastes tokens + causes 'lost-in-the-middle'", "Infinitely increases recall", "No issue"], explanationEn: "The LLM tends to miss information buried in a long context." },
          { question: "Embedding model nên cùng cho query và chunk?", options: ["Khác model OK", "Bắt buộc cùng model + version", "Tùy ý", "Tùy mood"], answer: 1, explanation: "Vector space khác nhau = similarity vô nghĩa.", questionEn: "Should the embedding model be the same for query and chunks?", optionsEn: ["Different models are fine", "Must be the same model + version", "Doesn't matter", "Depends on mood"], explanationEn: "Different vector spaces make similarity scores meaningless." },
        ],
      },
      {
        id: "edtech-ai-3",
...
"""`,
        ...
        quiz: [
          { question: "CoT giúp nhiều nhất với?", options: ["Dịch", "Toán/logic multi-step", "Format JSON", "Image gen"], answer: 1, explanation: "Step-by-step phân rã giảm sai sót.", questionEn: "CoT helps most with?", optionsEn: ["Translation", "Multi-step math/logic", "JSON formatting", "Image generation"], explanationEn: "Step-by-step decomposition reduces errors." },
          { question: "Few-shot nên?", options: ["Dùng schema khác cho mỗi ví dụ", "Cùng schema, đa dạng độ khó", "1 ví dụ duy nhất", "Không format"], answer: 1, explanation: "Đa dạng tránh overfit, cùng schema để bắt chước.", questionEn: "Few-shot examples should?", optionsEn: ["Use a different schema per example", "Share the same schema, vary in difficulty", "Use only one example", "Have no format"], explanationEn: "Diversity avoids overfitting; a shared schema helps imitation." },
          { question: "Self-consistency là?", options: ["Lấy 1 mẫu", "Lấy n mẫu rồi vote đáp án phổ biến nhất", "Dùng temperature=0", "Cache"], answer: 1, explanation: "Tăng độ chính xác cho câu hỏi khó.", questionEn: "Self-consistency means?", optionsEn: ["Taking one sample", "Taking n samples and voting on the most common answer", "Using temperature=0", "Caching"], explanationEn: "It improves accuracy on hard questions." },
          { question: "Anti-pattern?", options: ["Rõ ràng, ít rule", "Negation overload + 20 rules", "Few-shot 3 ví dụ", "Schema rõ"], answer: 1, explanation: "Model tập trung sai và bỏ qua.", questionEn: "Which is an anti-pattern?", optionsEn: ["Clear, few rules", "Negation overload + 20 rules", "Few-shot with 3 examples", "Clear schema"], explanationEn: "The model fixates on the wrong things and skips the rest." },
          { question: "ReAct kết hợp?", options: ["Reasoning + Acting (tool calls)", "Chỉ reasoning", "Chỉ tool", "Cache"], answer: 0, explanation: "Suy luận xen kẽ gọi tool.", questionEn: "ReAct combines?", optionsEn: ["Reasoning + Acting (tool calls)", "Reasoning only", "Tool calls only", "Caching"], explanationEn: "It interleaves reasoning with tool calls." },
        ],
      },
      {
        id: "edtech-ai-4",
...
Output JSON: {{"grounded": true|false, "unsupported_claims": [...]}}"""
    return llm_json(prompt, temperature=0)`,
        ...
        quiz: [
          { question: "LLM-as-judge nên dùng?", options: ["1 judge duy nhất", "Multi-judge + median", "Không cần judge", "Random"], answer: 1, explanation: "Giảm bias của 1 model.", questionEn: "How should LLM-as-judge be used?", optionsEn: ["A single judge", "Multiple judges + median", "No judge needed", "Randomly"], explanationEn: "Reduces the bias of any single model." },
          { question: "Citation grounding để?", options: ["Tăng độ trễ", "Buộc LLM trích sentence ID, chống bịa", "Trang trí", "SEO"], answer: 1, explanation: "Có ID = có thể verify.", questionEn: "What is citation grounding for?", optionsEn: ["Increasing latency", "Forcing the LLM to cite sentence IDs, preventing fabrication", "Decoration", "SEO"], explanationEn: "Having an ID means it can be verified." },
          { question: "Pedagogical hallucination là?", options: ["Sai dữ kiện", "Cho đáp án không phù hợp trình độ", "Latency cao", "PII leak"], answer: 1, explanation: "VD: dùng từ B2 dạy trẻ A1.", questionEn: "What is a pedagogical hallucination?", optionsEn: ["A wrong fact", "An answer unsuited to the learner's level", "High latency", "PII leak"], explanationEn: "E.g. using B2-level words to teach an A1 learner." },
          { question: "Test set cố định để?", options: ["Decoration", "So sánh phiên bản qua thời gian (regression)", "Không cần", "Random"], answer: 1, explanation: "Đổi prompt mà score tụt = regression.", questionEn: "What is a fixed test set for?", optionsEn: ["Decoration", "Comparing versions over time (regression testing)", "Not needed", "Random"], explanationEn: "If score drops after a prompt change, that's a regression." },
          { question: "Self-check chống được?", options: ["Latency", "Hallucination factual + source", "Cost", "Bug code"], answer: 1, explanation: "Hỏi lại model để xác nhận.", questionEn: "What does self-check help prevent?", optionsEn: ["Latency", "Factual + source hallucination", "Cost", "Code bugs"], explanationEn: "Asking the model again to confirm its own answer." },
        ],
      },
      {
        id: "edtech-ai-5",
...
def output_safe(text: str, tier: str) -> tuple[bool, str]:
    ...
    return True, "ok"`,
        ...
        quiz: [
          { question: "Defense in depth nghĩa là?", options: ["1 layer mạnh", "Nhiều layer độc lập (input, prompt, tool, output, audit)", "Không cần audit", "Random"], answer: 1, explanation: "Một layer bị bypass thì còn các layer khác.", questionEn: "What does defense in depth mean?", optionsEn: ["One strong layer", "Multiple independent layers (input, prompt, tool, output, audit)", "No audit needed", "Random"], explanationEn: "If one layer is bypassed, the others still catch it." },
          { question: "Kids tier KHÔNG nên có?", options: ["Encouragement", "Bạo lực + romance + PII", "Quiz", "Visuals"], answer: 1, explanation: "Trẻ em cần guardrail nghiêm nhất.", questionEn: "What should the Kids tier NOT have?", optionsEn: ["Encouragement", "Violence + romance + PII", "Quizzes", "Visuals"], explanationEn: "Children need the strictest guardrails." },
          { question: "Chống prompt injection?", options: ["Trust user", "Wrap markers + classifier riêng + system reaffirm", "Tăng temperature", "Tắt log"], answer: 1, explanation: "Cần nhiều layer.", questionEn: "How to defend against prompt injection?", optionsEn: ["Trust the user", "Wrap markers + dedicated classifier + system reaffirmation", "Increase temperature", "Turn off logging"], explanationEn: "Multiple layers are needed." },
          { question: "Bias audit nên kiểm?", options: ["Chỉ ngôn ngữ", "Tên, nghề, địa lý, tone", "Chỉ latency", "Không cần"], answer: 1, explanation: "Bias xuất hiện nhiều chiều.", questionEn: "What should a bias audit check?", optionsEn: ["Language only", "Names, job roles, geography, tone", "Latency only", "Nothing"], explanationEn: "Bias shows up across many dimensions." },
          { question: "PII trong output?", options: ["OK nếu nhỏ", "Tuyệt đối filter + audit", "Không cần", "Show toast"], answer: 1, explanation: "Lộ PII có thể vi phạm luật.", questionEn: "PII in output?", optionsEn: ["OK if minor", "Must be strictly filtered + audited", "No action needed", "Just show a toast"], explanationEn: "PII leaks can violate the law." },
        ],
      },
      {
        id: "edtech-ai-6",
...
async def speak_turn(audio_stream, history):
    ...
    async for transcript in stt_stream:
        ...
        quiz: [
          { question: "Latency speech tutor mục tiêu?", options: ["10s", "<2s end-to-end", "5s", "Không quan trọng"], answer: 1, explanation: "Trên 2s thì cuộc thoại không tự nhiên.", questionEn: "Target latency for a speech tutor?", optionsEn: ["10s", "<2s end-to-end", "5s", "Doesn't matter"], explanationEn: "Above 2s the conversation feels unnatural." },
          { question: "Chữ tay toán nên dùng?", options: ["Vision LLM trực tiếp", "MathPix → LaTeX → grader", "OCR thường", "Không hỗ trợ"], answer: 1, explanation: "MathPix chuyên biệt cho math.", questionEn: "For handwritten math you should use?", optionsEn: ["Vision LLM directly", "MathPix → LaTeX → grader", "Plain OCR", "Not supported"], explanationEn: "MathPix specializes in math." },
          { question: "Pronunciation Assessment đo gì?", options: ["Chỉ accuracy", "Accuracy + fluency + completeness + prosody", "Latency", "Cost"], answer: 1, explanation: "4 trục riêng biệt.", questionEn: "What does Pronunciation Assessment measure?", optionsEn: ["Accuracy only", "Accuracy + fluency + completeness + prosody", "Latency", "Cost"], explanationEn: "Four separate axes." },
          { question: "Browser Web Speech API?", options: ["Trả phí", "Miễn phí, OK cho casual, kém serious assessment", "Tốt nhất thế giới", "Không tồn tại"], answer: 1, explanation: "Trade-off cost vs quality.", questionEn: "The Browser Web Speech API is?", optionsEn: ["Paid", "Free, fine for casual use, weak for serious assessment", "The best in the world", "Nonexistent"], explanationEn: "A cost vs quality trade-off." },
          { question: "Streaming TTS giúp?", options: ["Tăng cost", "Giảm time-to-first-audio", "Tăng latency", "Không có lợi"], answer: 1, explanation: "Phát chunk audio ngay khi nhận.", questionEn: "Streaming TTS helps by?", optionsEn: ["Increasing cost", "Reducing time-to-first-audio", "Increasing latency", "No benefit"], explanationEn: "It plays audio chunks as soon as they arrive." },
        ],
      },
      {
        id: "edtech-ai-7",
...
def pick_prompt(feature: str, user_id: str) -> str:
    ...
    return cfg["canary"]["version"] if bucket < cfg["canary"]["percent"] else cfg["version"]`,
        ...
        quiz: [
          { question: "Mục đích shadow mode?", options: ["Marketing", "Chạy phiên bản mới song song, không ảnh hưởng user, để so sánh", "Tăng cost", "SEO"], answer: 1, explanation: "Test an toàn trước khi rollout.", questionEn: "Purpose of shadow mode?", optionsEn: ["Marketing", "Run the new version alongside the old, unaffecting users, for comparison", "Increasing cost", "SEO"], explanationEn: "A safe test before rollout." },
          { question: "Chiến lược cost?", options: ["Luôn dùng model lớn nhất", "Routing, caching, compression, batching, quota", "Không có cách", "Random"], answer: 1, explanation: "Combo nhiều kỹ thuật.", questionEn: "Cost strategy?", optionsEn: ["Always use the biggest model", "Routing, caching, compression, batching, quota", "No solution exists", "Random"], explanationEn: "A combination of multiple techniques." },
          { question: "Prompt regression nên?", options: ["Không test", "CI chạy eval set + block nếu giảm >2%", "Test thủ công vài lần", "Trust và deploy"], answer: 1, explanation: "Tránh release prompt làm giảm chất lượng.", questionEn: "How should prompt regression be handled?", optionsEn: ["No testing", "CI runs the eval set + blocks if score drops >2%", "A few manual tests", "Trust and deploy"], explanationEn: "Avoid shipping a prompt that lowers quality." },
          { question: "Rollback target time?", options: ["1 ngày", "<5 phút (feature flag)", "1 tuần", "Không cần"], answer: 1, explanation: "Production cần fast rollback.", questionEn: "Target rollback time?", optionsEn: ["1 day", "<5 minutes (feature flag)", "1 week", "Not needed"], explanationEn: "Production needs fast rollback." },
          { question: "Mastery lift đo?", options: ["Cost", "Mức tăng điểm pre vs post quiz - giá trị thực sự cho học sinh", "Latency", "Bug count"], answer: 1, explanation: "Đây là North Star metric của EdTech.", questionEn: "What does mastery lift measure?", optionsEn: ["Cost", "Score increase pre vs post quiz - the real value delivered to learners", "Latency", "Bug count"], explanationEn: "This is EdTech's North Star metric." },
        ],
      },
    ],
  },
