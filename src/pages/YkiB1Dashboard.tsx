...
const YkiB1Dashboard = () => {
  ...
  const [activeWritingCat, setActiveWritingCat] = useState<string>("opinion");
          ...
          <TabsContent value="writing" className="mt-6 space-y-4">
            {(() => {
              const categorize = (title: string): { key: string; label: string; emoji: string; color: string } => {
                ...
                if (t.startsWith("mielipide") || t.includes("mielipidekirjoitus")) return { key: "opinion", label: "Mielipide / Opinion", emoji: "💭", color: "bg-blue-500/10 text-blue-700 dark:text-blue-300 border-blue-500/30" };
                ...
                if (t.includes("uutinen")) return { key: "news", label: "Uutinen / News", emoji: "📰", color: "bg-slate-500/10 text-slate-700 dark:text-slate-300 border-slate-500/30" };
                if (t.includes("henkilökuvaus") || t.includes("henkilo")) return { key: "person", label: "Henkilökuvaus / Person", emoji: "👤", color: "bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border-indigo-500/30" };
              ...
              const activeGroup = groups.get(activeWritingCat) || ordered[0];

              return (
                <Card className="p-3 sm:p-4">
                  {/* Compact horizontal category filter */}
                  <div className="flex gap-1.5 flex-wrap mb-3 pb-3 border-b">
                    {ordered.map(g => {
                      const isActive = activeWritingCat === g.meta.key;
                      return (
                        <button
                          key={g.meta.key}
                          onClick={() => setActiveWritingCat(g.meta.key)}
                          className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-full border transition-all ${
                            isActive
                              ? "bg-[#003580] text-white border-[#003580] shadow-sm scale-105"
                              : `${g.meta.color} hover:opacity-80`
                          }`}
                        >
                          <span>{g.meta.emoji}</span>
                          <span>{g.meta.label}</span>
                          <span className="opacity-70 text-[10px] px-1 rounded bg-background/40">{g.items.length}</span>
                        </button>
                      );
                    })}
                  </div>

                  {/* Prompts grid for selected category only */}
                      ...
                      return (
                        <button
                          ...
                          className={`text-left text-xs px-3 py-2 rounded-lg border transition-all ${
                            active
                              ? "bg-[#003580] text-white border-[#003580] shadow"
                              : `${activeGroup.meta.color} hover:opacity-80 hover:scale-[1.02]`
                          }`}
                          title={w.title}
                        >
                          {shortTitle}
