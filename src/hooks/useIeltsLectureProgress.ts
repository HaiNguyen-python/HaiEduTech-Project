// Hook to manage IELTS lecture progress (completed + bookmarked) via database
...
export const useIeltsLectureProgress = () => {
        ...
        if (data) {
          ...
          try {
            ...
            if (toMigrate.length > 0) {
              await supabase.from("ielts_lecture_progress").insert(toMigrate);
              ...
              // Clean up localStorage if already migrated
              localStorage.removeItem(LOCAL_COMPLETED_KEY);
              localStorage.removeItem(LOCAL_BOOKMARKS_KEY);
            }
